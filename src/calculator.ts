 ...
 */
export function formatInput(args: CalculatorArgs): string {
  const lines = [
    `QPS: ${args.qps} K`,
    `Key-Value Size: ${args.size} bytes`,
    `Number of Keys: ${args.nkey} K`,
    `Connections: ${args.nconn}`,
    `Failure Domain: ${args.failureDomain.toFixed(1)}%`
  ];
  return lines.join('\n');
}

/**
 * Format cache config output (for segcache and rds)
 */
export function formatCacheOutput(config: SegcacheConfig | RdsConfig, runnable: "segcache" | "rds"): string {
  const label = runnable === "segcache" ? "pelikan_segcache" : "pelikan_rds";
  return `
${label} config:
  hash_power:      ${config.hashPower}
  seg_mem:         ${config.segMem} MB
...
/**
 * Format pingserver config output
 */
export function formatPingserverOutput(config: PingserverConfig): string {
  return `
pingserver config:
...
/**
 * Format output based on runnable type
 */
export function formatOutput(config: CalculatorConfig, runnable: Runnable): string {
  switch (runnable) {
    case "segcache":
    case "rds":
      return formatCacheOutput(config as SegcacheConfig | RdsConfig, runnable);
    case "pingserver":
      return formatPingserverOutput(config as PingserverConfig);
    default:
      return "";
  }
}
